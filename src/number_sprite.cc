...
namespace {
const std::string kNumberFile{"TextBox"};
...
NumberSprite::NumberSprite(Graphics &graphics, int number, int num_digits,
    ...
    : color_{color}, op_{op} {
  assert(number >= 0);
  const units::Game kNumberSourceY =
      color == ColorType::RED ? kSourceRedY : kSourceWhiteY;
  int digit_count{};
  do {
    const int digit = number % kRadix;

    reversed_glyphs_.push_back(std::make_shared<Sprite>(
        graphics, kNumberFile, units::gameToPixel(digit * kDigitSizeX),
        units::gameToPixel(kNumberSourceY), units::gameToPixel(kSourceWidth),
  ...
  padding_ =
      num_digits == 0 ? 0.0f : units::kHalfTile * (num_digits - digit_count);
  switch (op) {
  case OperatorType::PLUS:
    reversed_glyphs_.push_back(std::make_shared<Sprite>(
        graphics, kNumberFile, units::gameToPixel(kPlusSourceX),
        units::gameToPixel(kOpSourceY), units::gameToPixel(kSourceWidth),
        units::gameToPixel(kSourceHeight)));
    break;
  case OperatorType::MINUS:
    reversed_glyphs_.push_back(std::make_shared<Sprite>(
        graphics, kNumberFile, units::gameToPixel(kMinusSourceX),
        units::gameToPixel(kOpSourceY), units::gameToPixel(kSourceWidth),
        units::gameToPixel(kSourceHeight)));
    break;
  default:
...
void NumberSprite::draw(Graphics &graphics, units::Game x,
                        units::Game y) const {
  for (auto i{0uz}; i < reversed_glyphs_.size(); ++i) {
    const units::Game offset =
        units::kHalfTile * (reversed_glyphs_.size() - 1 - i);
