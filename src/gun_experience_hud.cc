...
void GunExperienceHUD::draw(Graphics& graphics, 
        ...
        units::GunExperience cur_experience,
        units::GunExperience level_experience){
    level_sprite_.draw(graphics, 
            units::tileToGame(kLevelDrawX),
            kDrawY);
    ...
    if (cur_experience < level_experience){
        fill_sprite_.set_percentage_width(static_cast<float>(cur_experience) / level_experience);
