...
Player::Player(units::Game x, units::Game y, Graphics &graphics)
    : x_{x}, y_{y}, velocity_x_{0.0f}, velocity_y_{0.0f}, acceleration_x_{0.0f},
      on_ground_{false}, horizontal_facing_{HorizontalFacing::LEFT},
      intended_vertical_facing_{VerticalFacing::HORIZONTAL},
      jump_active_{false}, interacting_{false}, health_{graphics},
...
void Player::updateY(units::MS elapsed_time_ms, const Map &map,
                     ParticleTools &particle_tools) {
  const units::Accelaration gravity{
      jump_active_ && velocity_y_ < 0.0f ? kJumpGravity : kGravity};
  velocity_y_ =
      std::min(velocity_y_ + gravity * elapsed_time_ms.count(), kMaxSpeedY);
  ...
  if (delta > 0.0f) {
    ...
    if (info.collided) {
      ...
      particle_tools.front_system.addNewParticle(
          std::make_shared<HeadBumpParticle>(particle_tools.graphics,
                                             center_x(), y_ + kCollisionYTop));
    }
  } else {
...
void Player::draw(Graphics &graphics) {
  if (spriteIsVisible()) {
    polar_star_.draw(graphics, x_, y_, horizontal_facing_, vertical_facing(),
                     gun_up());
...
void Player::stopMoving() { acceleration_x_ = 0.0f; }
...
void Player::lookDown() {
  if (intended_vertical_facing_ == VerticalFacing::DOWN)
    return;
...
void Player::stopFire() { polar_star_.stopFire(); }

void Player::stopJump() { jump_active_ = false; }
...
Rectangle Player::damageRectangle() const {
  return Rectangle{x_ + kCollisionX.left(), y_ + kCollisionYTop,
                   kCollisionX.width(), kCollisionYHeight};
}
      ...
      ENUM_FOREACH(vf, VERTICAL_TYPE, VerticalFacing) {
        ENUM_FOREACH(st, STRIDE_TYPE, StrideType) {
          initializeSprite(graphics,
                           std::make_tuple(static_cast<MotionType>(mt),
...
MotionType Player::motionType() const {
  ...
  if (interacting_) {
    ...
    motion =
        acceleration_x_ == 0.0f ? MotionType::STANDING : MotionType::WALKING;
  } else {
...
Player::SpriteState Player::getSpriteState() const {
  return SpriteState{std::make_tuple(motionType(), horizontal_facing_,
                                     vertical_facing(),
...
Rectangle Player::leftCollision(units::Game delta) const {
  assert(delta <= 0);
  return Rectangle{x_ + kCollisionX.left() + delta, y_ + kCollisionX.top(),
                   kCollisionX.width() / 2 - delta, kCollisionX.height()};
}

Rectangle Player::rightCollision(units::Game delta) const {
  assert(delta >= 0);
  return Rectangle{x_ + kCollisionX.left() + kCollisionX.width() / 2,
                   y_ + kCollisionX.top(), kCollisionX.width() / 2 + delta,
                   kCollisionX.height()};
}

Rectangle Player::topCollision(units::Game delta) const {
  assert(delta <= 0);
  return Rectangle{x_ + kCollisionTopLeft, y_ + kCollisionYTop + delta,
                   kCollisionTopWidth, kCollisionYHeight / 2 - delta};
}
...
Rectangle Player::bottomCollision(units::Game delta) const {
  ...
  return Rectangle{x_ + kCollisionBottomLeft,
                   ...
                   kCollisionBottomWidth, kCollisionYHeight / 2 + delta};
}
